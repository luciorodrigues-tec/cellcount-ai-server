...
export class AnalysisWorkerPool {
  constructor({
    ...
    sessionStore,
    executionBoundary,
    config = resolveAnalysisWorkerPoolConfig(),
    workerId = `${os.hostname()}-${process.pid}-${crypto.randomUUID().slice(0, 8)}`,
    logger = console,
    payloadDecoder = decodeAnalysisJobPayload,
  } = {}) {
  ...
  start() {
    ...
    this.running = true;
    this.loopPromises = Array.from({ length: this.config.concurrency }, (_, index) =>
      this.#loop(`${this.workerId}:${index + 1}`),
    );
  }
  ...
  async runOnce(workerId = `${this.workerId}:manual`) {
    const claim = await this.queue.claimNext({
      workerId,
      leaseTtlMs: this.config.jobLeaseTtlMs,
    });
  ...
  async #processClaim(claim, workerId) {
    ...
    try {
      ...
      if (!sessionClaim?.acquired) {
        ...
        if (retryable) this.metrics.retried += 1;
      ...
      heartbeat = setInterval(() => {
        Promise.all([
          ...
          this.sessionStore.renewExecutionLease(job.analysisId, job.userId, sessionLeaseToken, {
            leaseTtlMs: this.config.sessionLeaseTtlMs,
          }),
        ]).then(() => {
          this.metrics.leaseRenewals += 1;
        }).catch((error) => {
          heartbeatFailure ??= error;
          this.metrics.leaseRenewalFailures += 1;
        });
      ...
      const result = await this.executionBoundary.execute({
        ...
        context: {
          ...
          userId: job.userId,
          attempt: sessionClaim.session?.attempt ?? null,
      ...
      await this.sessionStore.markCompleted(job.analysisId, job.userId, result, {
        leaseToken: sessionLeaseToken,
      });
      await this.queue.markCompleted(job.jobId, jobLeaseToken);
      this.metrics.completed += 1;
      ...
      if (sessionLeaseToken) {
        try {
          const session = await this.sessionStore.markFailed(job.analysisId, job.userId, error, {
            leaseToken: sessionLeaseToken,
          });
      ...
      try {
        await this.queue.markFailed(job.jobId, jobLeaseToken, error, {
          retryable,
...
export function createAnalysisWorkerPool(options) {
  return new AnalysisWorkerPool(options);
}
